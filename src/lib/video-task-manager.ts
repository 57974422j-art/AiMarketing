  ...
  try {
    ...
    for (let b = 0; b < mp.length; b += 2) {
      await Promise.all(mp.slice(b, b + 2).map(async (src, bi) => {
        ...
        await run(`nice -n 19 ffmpeg -y -f image2 -framerate 25 -i "${src}" -vf "scale=${W}:${H}:force_original_aspect_ratio=1,pad=${W}:${H}:(ow-iw)/2:(oh-ih)/2,fade=t=in:st=0:d=0.5,fade=t=out:st=${(segDuration - 0.5).toFixed(2)}:d=0.5" -t ${segDuration.toFixed(2)} -c:v libx264 -preset fast -pix_fmt yuv420p -threads 2 "${c}"`, 60000)
      }))
    ...
    await run(`ffmpeg -y -i "${mv}" -i "${ai}" -vf "subtitles='${sp}':force_style='FontName=/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc:FontSize=${fs2},Alignment=2'" -c:v libx264 -preset medium -crf 23 -c:a aac -map 0:v -map 1:a -shortest -threads 2 "${op}"`, 180000)
    task.progress = 100
