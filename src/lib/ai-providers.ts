...
const DH_MODEL = process.env.DASHSCOPE_DIGITALHUMAN_MODEL || 'emo'
...
interface SC { width: number; height: number }
interface DR extends AIDecision { rawResponse?: string }

async function dd(milestone: string, goal: string, b64: string, scr: SC): Promise<DR | null> {
  const key = getDashScopeKey()
  if (!key) return null
  const p = `你是抖音自动化操作的视觉定位专家。请严格按规则输出 JSON。

【屏幕尺寸】${scr.width} x ${scr.height} 像素
【总目标】${goal}

【★ 强制执行流程 — 必须严格按顺序 ★】

正确的发布流程是：首页(home) → 点"+"进入拍摄页(shoot) → 点"相册"进入相册页(album) → 选视频 → 编辑页(edit) → 发布页(publish) → 完成

【判断当前页面类型，然后决定操作】
0️⃣ 手机桌面/主屏幕(非抖音)：未打开抖音App → 点击抖音图标或等待
1️⃣ 首页(home)：有底部导航栏(首页/朋友/+/消息/我)，中间有推荐视频流 → 点底部导航栏正中间的"+"号中心（这是发布入口！不是拍摄按钮）
2️⃣ 拍摄页(shoot)：有相机取景器/实时画面、大圆形白色拍摄按钮、"相册"文字在左下角 → 点左下角的"相册"文字中心（注意区分：圆形的是拍照按钮不要点！要点"相册"二字）
3️⃣ 相册页(album)：有"全部/视频/图片"标签 + 视频/图片缩略图网格 → 点左上角第一个视频缩略图的中心
4️⃣ 编辑页(edit)：有视频预览 + "添加标题"灰色占位文字 + 编辑工具栏 → 点"添加标题"区域中心准备输入
5️⃣ 发布页(publish)：有视频预览 + "发布"或"发作品"高亮按钮 → 点"发布"/"发作品"按钮中心
6️⃣ 弹窗(popup)：有半透明遮罩层 + 弹出提示框 → 点弹窗上的"我知道了"/"去编辑"/"允许"/"取消"
7️⃣ 成功完成：看到"发布成功"等确认信息 → status="DONE"

【重要提醒】
- ⚠️ 如果当前页面不在预期顺序上（比如一上来就在拍摄页），优先回到首页重新开始流程
- ⚠️ 首页的"+"和拍摄页的"拍摄按钮"是完全不同的东西！首页"+"在导航栏里，拍摄页的大圆钮是相机快门
- ⚠️ 拍摄页的"相册"文字通常在屏幕左下角，不是底部的圆形按钮
...
function pr(rt: string, scr: SC): DR | null {
  let js = rt; const m = rt.match(/\{[\s\S]*"analysis"[\s\S]*\}/)
  if (m) js = m[0]
  js = js.replace(/\x60\x60\x60json\n?/g, '').replace(/\x60\x60\x60\n?/g, '').replace(/,(\s*[\}\]])/g, '$1')
  let d; try { d = JSON.parse(js) } catch { return null }
  if (!d.analysis || !d.status || !d.action) return null
  let c = { x: 0, y: 0 }
  if (d.coordinates) { let rx = d.coordinates.x, ry = d.coordinates.y; if (rx > 0 && rx <= 1 && ry > 0 && ry <= 1) { rx = Math.round(rx * scr.width); ry = Math.round(ry * scr.height) } c = { x: Math.min(Math.max(Math.round(rx), 0), scr.width), y: Math.min(Math.max(Math.round(ry), 0), scr.height) } }
  return { analysis: d.analysis, status: d.status === 'DONE' ? 'DONE' : 'CONTINUE', action: d.action === 'input' ? 'input' : d.action === 'wait' ? 'wait' : 'click', target_desc: d.target_desc || '', coordinates: c, text_content: d.text_content || '' }
}

export async function aiDecideNext(b64: string, milestone: string, goal: string, scr: SC = { width: 1080, height: 2340 }): Promise<AIDecision | null> {
  if (!b64) return null
  const r = await dd(milestone, goal, b64, scr)
  if (!r) return { analysis: 'AI决策失败', status: 'CONTINUE', action: 'wait', target_desc: '', coordinates: { x: 0, y: 0 }, text_content: '' }
  return r
}
...
const locatorPrompts: Record<string, string> = {
  '加号': `在抖音App截图上找到底部导航栏中间的"+"号图标（不是文字"加号"，是一个+符号）。
特征：灰白色圆形背景框里面有一个"+"符号，位于屏幕底部导航栏的正中央位置。
左右两侧分别是"朋友"和"消息"导航项。这个"+"号比其他图标稍大一些。
注意：坐标必须在屏幕底部区域（y值应该大于屏幕高度的85%），如果返回的y值太小说明找错了。
...
只输出 JSON：{"x": 整数像素, "y": 整数像素}。找不到输出 {"x": -1, "y": -1}。`,
  '下一步': `在抖音相册页截图中找到屏幕底部右侧的"下一步"按钮中心坐标。
特征：红色或亮红色圆角矩形按钮，上面写白色大字"下一步"，位于屏幕底部导航栏区域（y值应大于屏幕高度的85%），通常在右下角位置，旁边可能有"选择"和"一键成本"等文字按钮。
注意：不要返回顶部区域的坐标，y必须大于屏幕高度的80%。找不到输出 {"x": -1, "y": -1}。
只输出 JSON：{"x": 整数像素, "y": 整数像素}。`,
}
