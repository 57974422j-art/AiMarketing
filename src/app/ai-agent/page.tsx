    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        if (generated.trainingDocuments && generated.trainingDocuments.length > 0) {
          showToast('AI已帮您解析需求！请检查并确认生成的员工信息。');
        }
      } else {
        showToast(data.message || 'AI解析失败，请重试');
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <p className="text-label mb-2">AI 员工管理</p>
            <h1 className="text-mono-lg text-white">智能体</h1>
          </div>
          ...
          {agents.map(agent => (
            <div key={agent.id} className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
              ...
              <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                <span>{agent.trainingDocuments.length} 份文档</span>
                ...
                <div>
                  ...
                  >
                    {replyStyles.map(style => (
                      <option key={style.cn} value={style.cn} className="bg-gray-900">{style.cn}</option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  <div>
                    ...
                    >
                      {documentTypes.map(type => (
                        <option key={type.cn} value={type.cn} className="bg-gray-900">{type.cn}</option>
                      ))}
                    </select>
                  </div>
                </div>
