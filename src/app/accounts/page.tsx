...
export default function AccountsPage() {
  ...
  const [browserInputText, setBrowserInputText] = useState('')
  const [browserClickPos, setBrowserClickPos] = useState<{ x: number; y: number } | null>(null)
  ...
  const handleScreenshotClick = (e: React.MouseEvent<HTMLImageElement>) => {
    ...
    // 截图原始尺寸 1280x800，按比例换算坐标
    const scaleX = 1280 / rect.width
    const scaleY = 800 / rect.height
    const x = Math.round((e.clientX - rect.left) * scaleX)
    const y = Math.round((e.clientY - rect.top) * scaleY)
    setBrowserClickPos({ x, y })
  }
                  ...
                  {!workbench.accountId ? (
                    ...
                    <>
                      ...
                      <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 flex items-center justify-between">
                        <div>
                          <p className="text-emerald-300 text-sm font-medium">🟢 浏览器运行中</p>
                          <p className="text-[11px] text-emerald-400/70">端口 {workbench.accountId} · 点击截图可定位，然后输入或点击</p>
                        </div>
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400">可操作</span>
                      </div>

                      {/* 可点击截图预览 */}
                      <div className={`relative rounded-xl overflow-hidden border ${browserClickPos ? 'border-cyan-500/50' : 'border-white/5'} cursor-crosshair`}>
                        {browserScreenshot ? (
                          <img
                          ...
                          <div className="w-full aspect-[16/10] bg-gray-900 flex items-center justify-center">
                        ...
                        {browserClickPos && (
                          <div
                            className="absolute w-4 h-4 -ml-2 -mt-2 rounded-full bg-cyan-400/80 animate-pulse ring-2 ring-cyan-400"
                            style={{ left: `${(browserClickPos.x / 1280) * 100}%`, top: `${(browserClickPos.y / 800) * 100}%` }}
                          />
                        )}
                        {/* 加载遮罩 */}
                        {browserActionLoading && (
                          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                            <p className="text-white text-sm animate-pulse">执行中...</p>
                          </div>
                        )}
                      </div>
                      ...
                      {/* 坐标显示 */}
                      {browserClickPos && (
                        <p className="text-[11px] text-cyan-400 text-center">已选择坐标: ({browserClickPos.x}, {browserClickPos.y}) · 输入文字后点"输入"发送</p>
                      )}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            placeholder="输入要发送的文字（手机号、验证码等）..."
                            className="flex-1 bg-gray-800/50 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder-gray-500 focus:border-cyan-500/50 focus:outline-none"
                            onKeyDown={(e) => e.key === 'Enter' && browserClickPos && sendBrowserAction('type', { x: browserClickPos.x, y: browserClickPos.y, text: browserInputText })}
                          />
                          <button
                            onClick={() => browserClickPos && sendBrowserAction('type', { x: browserClickPos.x, y: browserClickPos.y, text: browserInputText })}
                            disabled={!browserClickPos || !browserInputText.trim() || browserActionLoading}
                            className="px-3 py-2 bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 rounded-lg text-xs hover:bg-cyan-500/30 disabled:opacity-30 disabled:cursor-not-allowed transition whitespace-nowrap"
                          >
                            ⌨️ 输入
                          </button>
                        </div>
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          <button
                            onClick={() => browserClickPos && sendBrowserAction('click', { x: browserClickPos.x, y: browserClickPos.y })}
                            disabled={!browserClickPos || browserActionLoading}
                            className="py-2 bg-blue-500/20 text-blue-400 border border-blue-500/30 rounded-lg text-xs hover:bg-blue-500/30 disabled:opacity-30 disabled:cursor-not-allowed transition"
                          >
                            👆 点击
                          </button>
                          <button
                            onClick={() => sendBrowserAction('enter')}
                            disabled={browserActionLoading}
                            className="py-2 bg-green-500/20 text-green-400 border border-green-500/30 rounded-lg text-xs hover:bg-green-500/30 disabled:opacity-30 disabled:cursor-not-allowed transition"
                          >
                            ↵ 回车
                          </button>
                          <button
                            onClick={() => sendBrowserAction('screenshot')}
                            disabled={browserActionLoading}
                            className="py-2 bg-orange-500/20 text-orange-400 border border-orange-500/30 rounded-lg text-xs hover:bg-orange-500/30 disabled:opacity-30 disabled:cursor-not-allowed transition"
                          >
                            📸 刷新
                          </button>
                          <button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                if (res.ok) {
                                  ...
                                  setBrowserScreenshot(null)
                                  setBrowserInputText('')
                                  setBrowserClickPos(null)
                                }
                              } catch (_) { setBrowserStatus('running') }
                            }}
                            disabled={browserActionLoading}
                            className="py-2 bg-red-500/20 text-red-400 border border-red-500/30 rounded-lg text-xs hover:bg-red-500/30 disabled:opacity-30 disabled:cursor-not-allowed transition"
                          >
                            ⏹ 停止
                          </button>
                        </div>
                        ...
                        {/* 提示信息 */}
                        {browserMsg && (
                          <p className="text-[11px] text-yellow-400 text-center">{browserMsg}</p>
                        )}

                        {/* 使用说明 */}
                        <p className="text-[10px] text-gray-600 text-center leading-relaxed">
                          使用方法：1.点击截图上的目标位置 2.在输入框填写内容 3.点击"输入"或"点击"按钮
                        </p>
                      </div>
                    </>
                  )}
