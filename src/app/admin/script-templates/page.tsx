...
export default function AdminScriptTemplatesPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) { setShowForm(false); loadItems(); showToast(editItem ? '更新成功' : '创建成功', 'success'); }
  ...
  const generateMockScripts = (prompt: string, scriptType: string): string[] => {
    const templates: Record<string, string[]> = {
      ...
      '私信': [
        `【${prompt}-私信】您好！很高兴认识您 😊 我是您的专属顾问。请问您是想了解哪方面内容呢？\n\n我可以根据您的需求给您推荐最适合的方案~`,
        `【${prompt}-私信】亲爱的，收到您的咨询啦！这边帮您整理了一份详细资料，包括：\n✅ 产品介绍\n✅ 价格方案\n✅ 真实案例\n\n您方便的时候我发给您看看？`,
      ],
      '直播互动': [
        `【${prompt}-直播】欢迎新进来的宝宝！今天直播有超多福利🎁 想了解XX的扣1，想看效果的扣2~`,
        `【${prompt}-直播】有人问到XX好不好用，我来给大家实拍一下！看清楚了哈...（演示）怎么样是不是很心动？今天下单还有优惠！`,
        `【${prompt}-直播】宝宝们别急啊，一个个来！客服在后台回复大家呢～ 先给大家发个红包🧧抢到了记得回来下单哦！`,
      ],
      '欢迎语': [
        `【${prompt}-欢迎】欢迎宝宝加入我们的大家庭！🎉\n\n这里是XX官方账号，每天分享：\n💡 行业干货\n⭐ 好物推荐\n🎁 粉丝专属福利\n\n有任何问题随时私信我哦～`,
      ],
      '售后': [
        `【${prompt}-售后】亲，收到您的反馈了！非常抱歉给您带来不好的体验 😔\n\n这边马上帮您处理：\n1. 订单号发给我确认一下\n2. 我安排专员跟进\n3. 预计24小时内给您满意答复\n\n感谢您的理解和支持！`,
      ],
    };

    // 如果有精确匹配则使用，否则基于 prompt 动态生成
    let result = templates[scriptType];
    if (!result) {
      result = [
        `【${prompt}-${scriptType}】这是为您定制的话术模板第一条，可以根据实际情况灵活调整使用~`,
        `【${prompt}-${scriptType}】这是为您定制的话术模板第二条，建议配合表情包一起发送效果更佳！`,
        `【${prompt}-${scriptType}】这是为您定制的话术模板第三条，适用于高频场景的快速回复~`,
      ];
    }

    return result.map((s, i) => `${s}\n\n--- AI 生成 #${i + 1} | 场景：${prompt} | 类型：${scriptType} ---`);
  };
  ...
  if (!user || user.role === 'end-user') return <div className="min-h-screen bg-gray-950 flex items-center justify-center"><p className="text-red-400 text-xl">无权限访问</p></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setShowAIPanel(!showAIPanel)}
              className={`px-4 py-2 rounded-xl font-mono text-sm transition-colors flex items-center gap-2 ${
                showAIPanel
          ...
          <div className="bg-gradient-to-br from-purple-500/10 to-blue-500/10 backdrop-blur-sm rounded-2xl border border-purple-500/20 p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <select value={aiScenario} onChange={e => { setAiScenario(e.target.value); }}
                  className="input-dark" size={4}
                >
                  <option value="" className="bg-gray-900">-- 选择预设场景 --</option>
                  {AI_SCENARIOS.map(s => <option key={s} value={s} className="bg-gray-900">{s}</option>)}
                </select>
              </div>
              ...
              <div>
                ...
                <textarea value={aiCustomPrompt} onChange={e => setAiCustomPrompt(e.target.value)}
                  className="input-dark h-24 resize-y" placeholder="描述你的产品/服务、目标客户、期望语气..." />
              </div>
            </div>
                ...
                {SCRIPT_TYPES.map(t => (
                  <button key={t.id} onClick={() => setType(t.id)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-mono transition-colors ${
                      type === t.id ? 'bg-purple-500 text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10'
                    }`}>
                    {t.icon} {t.id}
                  </button>
                ))}
            ...
            <div className="flex justify-end">
              ...
              >
                {aiGenerating ? (
                  <><span className="animate-spin">⟳</span> GENERATING...</span>
                ) : (
                  <><span>🚀</span> GENERATE SCRIPTS</span>
                )}
              </button>
            </div>
              ...
              <div className="mt-6 space-y-3">
                ...
                {aiResults.map((r, i) => (
                  <div key={i} className="bg-black/30 rounded-xl p-4 border border-white/10 group relative">
                    ...
                    <button onClick={() => applyAIResult(r)}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <div className="flex gap-1 overflow-x-auto pb-1">
            <button onClick={() => setFilterType('all')}
              className={`px-3 py-2 rounded-lg text-xs font-mono whitespace-nowrap ${filterType === 'all' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-gray-400'}`}>ALL</button>
            {SCRIPT_TYPES.map(t => (
          ...
          <div className="card-glass p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <select className="input-dark" value={type} onChange={e => setType(e.target.value)}>
                {SCRIPT_TYPES.map(t => <option key={t.id} value={t.id} className="bg-gray-900">{t.icon} {t.id}</option>)}
              </select>
              <input className="input-dark md:col-span-2" placeholder="标签（逗号分隔）" value={tagsStr} onChange={e => setTagsStr(e.target.value)} />
            </div>
            <textarea className="input-dark mb-4 h-40 resize-y" placeholder="话术内容 *" value={content} onChange={e => setContent(e.target.value)} />
            <div className="flex gap-3">
        ...
        {/* ====== 话术列表 ====== */}
        {loading ? (
          <div className="text-center text-gray-400 py-12"><div className="animate-spin w-8 h-8 border-2 border-emerald-500 border-t-transparent rounded-full mx-auto" /></div>
              ...
              <div key={item.id} className="card-glass p-6 group hover:border-emerald-500/20 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEdit(item)} className="px-3 py-1 text-xs bg-white/10 text-gray-300 rounded hover:bg-white/20 font-mono">EDIT</button>
                    <button onClick={() => handleDelete(item.id)} className="px-3 py-1 text-xs bg-red-500/20 text-red-400 rounded hover:bg-red-500/30 font-mono">DEL</button>
                  </div>
                </div>
