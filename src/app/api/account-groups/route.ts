...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getVisibleGroups(auth)
  ...
  try {
    ...
    if (body.name && !body.accountId) {
      const group = await prisma.accountGroup.create({
        data: { name: body.name, ownerId: auth.userId },
      })
      return NextResponse.json({ success: true, data: group }, { status: 201 })
    }

    // 添加成员到分组
    if (body.groupId && body.accountId) {
      ...
      const item = await prisma.accountGroupItem.upsert({
        where: { groupId_accountId: { groupId: body.groupId, accountId: body.accountId } },
        update: {},
        create: { groupId: body.groupId, accountId: body.accountId },
      })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (groupId && accountId) {
      const item = await prisma.accountGroupItem.findFirst({ where: { groupId, accountId } })
