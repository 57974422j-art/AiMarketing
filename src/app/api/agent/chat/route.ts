                ...
                try {
                  ...
                  if (vPick) {
                    ...
                    try {
                      ...
                      let covN = ''
                      if (visN) { try { const covR = await dashscopeGenerateImageAsync((visN.slice(0, 200) + '，营销封面风格，标题文字：' + kwN).trim(), '768*1344').catch(() => null); if (covR?.taskId) { let pi2 = 0; while (pi2 < 60) { pi2++; await new Promise((res) => setTimeout(res, 5000)); const qt = await fetch('https://dashscope.aliyuncs.com/api/v1/tasks/' + covR.taskId, { headers: { Authorization: 'Bearer ' + process.env.DASHSCOPE_API_KEY } }).then((r) => r.json()).catch(() => null); if (qt?.output?.task_status === 'SUCCEEDED') { const u = qt.output.results?.[0]?.url; if (u) { covN = u; break } } else if (qt?.output?.task_status === 'FAILED') break } } } catch {} }
