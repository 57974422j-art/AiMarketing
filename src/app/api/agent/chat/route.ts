...
const AGENT_TOOLS: ToolDefinition[] = [
    ...
    },
  {
    ...
    description: '发布/准备发布内容到自媒体平台（抖音/小红书/快手/视频号/B站）。触发词："发布""发抖音""发小红书""投稿""上传视频"。**不需要账号登记/绑定——只要用户给了视频文件名(videoName)和文案(caption)就直接创建发布任务**，客户端自动执行；**通道分流：抖音/小红书/微博走 OpenCLI（驱动用户已登录 Chrome 真发，需装 OpenCLI+扩展）；快手/视频号/B站走指纹浏览器**；执行时若未登录会提示。**前缀区分：用户说"打开指纹浏览器/打开发布"是跳转页面（open_page /my-fingerprint），不是立即发布——禁止调用本工具。**',
    parameters: {
      type: 'object',
      properties: {
...
"发布任务已创建（#数字ID）。客户端指纹浏览器页会自动启动浏览器并执行发布。查进度直接说「查发布状态」。"
【发布状态规则（硬规则）】
- **AI 无法看到执行过程**（上传中/填标题/点发布/预计时间全部看不到）——**禁止编造任何执行细节和进度**
