...
const kwM = vdT.match(/[“"\「『]([^”"\」』]{2,20})[”"\」』]/) || vdT.match(/名为[::：]?\s*([^，。；\n]{2,20})/) || vdT.replace(/内容总结|主体|场景|动作|文字|总结要素|\*\*/g, '').match(/(AI|AR|VR|短视频|营销|工具|平台|创作|展示)[^，。；\n]{0,12}/) || vdT.match(/([^，。；\n]{4,16})/)
                      ...
                      let covN = ''
                      try { const gen = await generateImage((visN.slice(0, 200) + '，营销封面风格，标题文字：' + kwN).trim() || '营销封面', '768*1344', 'auto'); if (gen?.url) { try { const cRes = await fetch(gen.url).catch(() => null); if (cRes?.ok) { const cBuf = Buffer.from(await cRes.arrayBuffer()); const cKey = 'storage/' + auth?.userId + '/cover_' + Date.now() + '.jpg'; await putObject(cKey, cBuf, 'image/jpeg'); covN = 'https://ai-niuma.cc/api/storage/file?name=' + cKey.replace('storage/' + auth?.userId + '/', '') + '&persist=1'; try { await prisma.mediaAsset.create({ data: { userId: auth?.userId || 0, type: 'image', name: '封面_' + Date.now(), ossUrl: covN, source: 'ai-cover' } }).catch(() => {}) } catch {} } } catch {} } } catch {}
