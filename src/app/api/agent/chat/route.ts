                ...
                try {
                  ...
                  if (vPick) {
                    ...
                    try {
                      ...
                      let covN = ''
                      if (visN || kwN) { try { const covR = await dashscopeGenerateImageAsync((visN.slice(0, 200) + '，营销封面风格，标题文字：' + kwN).trim() || '营销封面', '768*1344'); if (covR?.taskId) { const taskIdC = covR.taskId; setTimeout(async () => { try { let pi3 = 0; while (pi3 < 60) { pi3++; await new Promise((r) => setTimeout(r, 5000)); const qt = await fetch('https://dashscope.aliyuncs.com/api/v1/tasks/' + taskIdC, { headers: { Authorization: 'Bearer ' + process.env.DASHSCOPE_API_KEY } }).then((r) => r.json()).catch(() => null); if (qt?.output?.task_status === 'SUCCEEDED') { const u = qt.output.results?.[0]?.url; if (u) { try { const dm3 = await prisma.agentMemory.findFirst({ where: { userId: String(auth?.userId || 0), tags: { contains: 'pub_draft' } } }); if (dm3) { const dp3 = JSON.parse(String(dm3.content).replace(/^发布草稿:/, '') || '{}'); await prisma.agentMemory.update({ where: { id: dm3.id }, data: { content: '发布草稿:' + JSON.stringify({ ...dp3, coverUrl: u }) } }) } } catch {} break } } else if (qt?.output?.task_status === 'FAILED') break } } catch {} }, 10) } } catch {} }
