...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (provider) {
      ...
      case 'vvhan': {
        // 2026-08-14：vvhan 已移除——统一测免 key 聚合源
        try {
          ...
          return NextResponse.json({ valid: true, message: `免key源可用，返回 ${fn} 条微博热榜` });
        } catch (e: any) {
          return NextResponse.json({ valid: false, message: `免key源请求失败：${e.message}` });
        }
      }
