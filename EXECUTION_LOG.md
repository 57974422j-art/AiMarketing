...
| 2026-09-22 | **★用户数据安全两条铁律落地 + 多账号用户名回填（用户定案「以后不动就行了」）** → **背景**：用户发现"更新 207 后登录态丢失"，并提到更早还见过 `E:\ai-marketing\storage` 被删；定下两条原则：**登录态与本地仓库一律不许删；一台机器支持多账号，每账号各自一套数据**。**① 定位（查证结论）**：**应用自身代码从不删用户数据**（`storage/` 只写不删，`unlinkSync` 仅用于临时文件 / `.loggedin` 标记）；会删的是**打包/安装侧两处** —— ㈠ `scripts/build-local.mjs:140` 原来 `rmSync(整个 dist-rel/win-unpacked)`：客户端若正从该目录运行，其 **exe 同级 `data/`（多账号登录态 `data/browser-profile/{账号Id}/` + `accounts.json` + 指纹 profile）、`storage/`（本地仓库）、`python/`（内置环境）会被一并删**；㈡ `electron/installer.nsh` 的 `customRemoveFiles` 用 `for /d %i … @if /i not "%i"=="$INSTDIR\data" … rd /s /q "%i"` **靠路径字符串比较"排除"**用户目录 —— 路径展开只要有差异（尾斜杠 / 空格 / 短名）就比较失败 → 用户目录与程序文件一起被删；且**更新时执行的是旧版本带过来的卸载器**，从更旧版本升上来照样全删。另确认一个"看似丢数据"的常见误解：`userData` 指向 exe 同级 `data/` → **同一程序放在不同目录 = 两套完全独立的数据**（用户"换目录跑就登录态全没"多半是看了另一个目录，数据仍在旧目录）。**② 改法（白名单删除，不再"排除"）**：`installer.nsh` 只做 3 个动作 —— `del /q "$INSTDIR/*.*"`（程序根文件）+ `RMDir /r locales` + `RMDir /r resources`；用户数据都是目录，天然碰不到，**不再需要任何排除逻辑**（更新与卸载两条路一致，都保数据）；`build-local.mjs` 逐项删除并**跳过 `data` / `python` / `storage`**、打印「★保留用户数据」，单项被占用也不中断（交给 electron-builder 覆盖）。**③ 多账号用户名回填 `ACCOUNT_NAME_BACKFILL_V1`**：账号名来自 `accounts.json` 里 token 的 JWT username，而该文件原来**只在启动自检 ⑤ 写一次** → 刚登录 / 刚切换的账号只能显示编号 → 抽出 `recordCurrentAccount()`，在 **自检 ⑤ / `pick-account` 切换后 / 主窗口每次 `did-finish-load`（覆盖"刚登录完页面重载"）** 三处回填（token 只 base64 解码不验签 → **过期 token 也照样解出用户名**）；拿不到凭据的账号文案改为「账号 N（本机没存登录凭据——登录一次即可显示用户名）」。**④ 文档**：`AGENTS.md` 新增**硬规则 7（数据安全铁律，含多账号）**、`PROJECT.md` 打包前必查清单新增第 8 条、`ISSUES.md` 两条转「已解决」。 | `electron/installer.nsh`、`scripts/build-local.mjs`、`electron/main.js`、`AGENTS.md`、`PROJECT.md`、`ISSUES.md`、`electron/changelog.json` | ✅ `node --check` 逐个通过（`build-local.mjs` / `electron/main.js`）；`changelog.json` 仍为合法 JSON（207 条目 5 条）；✅ 抖音封面引导层修复已随本版打包（包内脚本与仓库 **md5 逐字节一致**）；⚠️ **installer.nsh 的语法只能由 NSIS 编译器在实际打包时验证** → 等用户重打包；⚠️ **必须重新打包 1.0.207**（当前那个 exe 仍是旧安装器，装上去照样删用户数据）；⚠️ 本轮已按用户授权 push |
| 2026-09-21 | **★修两处（用户实测 AI 制片第二轮 21:58→22:07）＋ 确认上一轮生效** → **✅ 先确认**：部署后用户实测，AI 制片分镜**已不再出现 `bgimage`**（变成 `title / end`）= `VF_AIONLY_V1` **生效**。**① "查询进度"被块外兜底误抢**：用户那句「请调用 query_video_task 查询任务 vf… 的最新进度」里**带着 MAKE_VIDEO_TASK 的整段文案**（含"本地成片已在后台开始 / 视频做得怎么样了"）→ 命中 `兜底2` 的 `/做.{0,4}视频\|成片\|做视频/` → 回了「这条视频用什么素材？」（**完全不相干的素材来源卡**）→ 看起来像"状态机又乱接"，**其实与素材无关**（`STD_QUERY_V1`：查询/进度类消息一律不触发兜底）；**② AI 制片文案没有"按目标时长"的护栏**：用户选 **10 秒**，AI 写了 **83 字（≈18 秒）** → 成片预计 25 秒（`VF_AIDUR_V1`：补上与素材线同口径的两道 —— 超 1.15 倍让 AI 压缩、仍超 1.35 倍按句末硬截到 1.1 倍；中文 4.5 字/秒 → 字数就是时长，必须卡在文案这步）。 | `src/app/api/agent/chat/route.ts`、`src/lib/agent/vf/vf-aivideo.ts` | ✅ `npx tsc --noEmit` **0 新增错误**（route.ts 仅剩 2 条历史遗留）；⚠️ 待实测：问「视频做得怎么样了」应回进度（不再出素材卡）；选 10 秒应出 ≈10~12 秒的片 |
| 2026-09-21 | **★按用户定案重做 AI 制片的"画面来源"边界：AI 制片 = 文生视频（除了第一步猜题材，全程不碰素材库）** → **用户定案原话**：「不要什么回退 AI制作就是AI制作，不要什么回退素材合成……**AI制片就是除了第一步看用户，其它都不拿个人仓库素材的**。失败了就是AI制作失败了。不要什么降级，降级我没有 minimax 官方 API 顶着呢。**这里就是一个文生视频。没有素材**」。**落地（`VF_AIONLY_V1` + `VF_AIFAIL_V1`）**：① **`genVideoShots` 加 `aiOnly`** —— AI 制片线归一化时**只允许"不需要素材图"的卡型**（title / list / number / compare / chart / end），`bgimage`/`image` **一律降级成 title**（不配图）→ 从源头保证"不会出现要素材图却没有图的镜"；② **AI 线一路不再喂素材** —— 分镜 `imgPaths: []` + `brief: ''`，**写文案的 prompt 也去掉素材摘要**（题材由第一步"看素材猜出的主题"承载）；③ **`make.py`：AI 制片必须每一镜都是 AI 画面** —— 缺任何一镜（生成失败/下载失败）**不用素材顶、不降级 = 直接判失败**（`sys.exit(5)`，并指向 `[H3]` 行说清病因）；混合线仍保留回退（它本来就有素材画面）、素材线完全不受影响；④ 顺手把 `[H3]` 里"→ 回退用原画面"这句误导文案改成"该镜算失败（AI 制片线）"。 | `src/app/api/agent/chat/route.ts`、`src/lib/agent/vf/vf-aivideo.ts`、`scripts/video-factory/make.py` | ✅ `npx tsc --noEmit` **0 新增错误**（route.ts 仅剩 2 条历史遗留）；✅ `python -m py_compile` 通过；⚠️ **一处待钉死**：本次 AI 制片分镜卡上出现了 `bgimage`，但按代码（`imgs=[]` 时无图必降级 title）**不该出现** → 需要该任务的 plan（`storage/<uid>/video-factory/vf1789997211542.json`）确认；本轮已从【源头（aiOnly）+ 归一化 + 出片判定】三头堵死，无论原因为何都不会再依赖素材 |
| 2026-09-21 | **★修「AI 制片出片必失败」的两处真 bug（用户实测任务 vf1789997211542，跑了 417s）** → **现象**：AI 制片**流程全通**（主题卡 → 文案卡 → 4 镜分镜卡 → 确认 → 入队 ✅），但出片失败：`[H3] 第 4 镜下载失败: HTTP Error 403: Forbidden` → `[H3] 生成完成 0/4 镜（通道=中转）` → `[MAKE] 回退素材合成` → `RuntimeError: 第 0 镜渲染失败(bgimage) … : No such file or directory`。**用户点破真问题**：「我做的 10 秒视频 好合成干嘛？AI 直接不就出了」。**根因（2 处）**：① **`make.py` 的 `_h3_download` 没带 UA** —— 我上一轮只给 API 调用（`_h3_http`）补了 UA，**产物"下载"这条路漏了** → 同样被 Cloudflare 拦 → **生成成功却一镜都拿不回来**（`H3_UA_V1`）；② **"回退素材合成"这个护栏对 AI 制片线是错的** —— 它本意是"单镜失败别让整片挂"（对素材线/混合线合理），但 **AI 制片没有素材**（本次仓库 0 张图）→ 回退后 `bgimage` 卡没有图片路径 → render.py 抛 `No such file or directory` → **整片失败**，还把真病因（下载 403）埋在"渲染错误"底下，极具误导性（`VF_AIFAIL_V1`：**AI 制片线一镜都没成功 → 不回退、直接失败并说清病因**；混合线仍保留回退，因为本线本来就有素材画面）。**顺带加固 + 诊断**：③ `render.py` 的 `card_bgimage` **无图 / 图不存在 → 降级为 title 卡**（`VF_NOIMG_V1`，原则同 `VF_UNKNOWNCARD_V1`：宁可这一镜样式朴素，也不要整条视频挂掉）；④ 下载失败时**把失败的 URL 打出来**（`H3_DLDIAG_V1`，原来只有 "HTTP 403: Forbidden"，换 key/换站时无法验证）。 | `scripts/video-factory/make.py`、`scripts/video-factory/render.py` | ✅ `python -m py_compile` 三个成片脚本语法通过；⚠️ **Python 改动 → 服务器 `git reset` 即生效（不用 build）**；⚠️ **待用户实测**：重跑一条 AI 制片，期望 `[H3] ✅ 第 N 镜 OK 中转` 并正常出片 |
| 2026-09-21 | **★★【标准模式锁死】按用户定案落地：命令白名单 + 单一路由器（批次 1，用户上线前不部署先讨论）** → **用户定案原话**：「请记住 11 条就是命令 少一个字 错一个字都不执行」「小红书搜索删除；其它三个点击出现开发中；先把手上有搞好」「标准模式就给我锁死，每一个独立状态机。点哪个进哪个；进去了任何一条，不管到 1.2.3.4.5 步，看到第一条就是重来」「除了这些按键，发什么文字 都可以回复没有这个功能，请切换去自由模式尝试或联系客服咨询」「这些虽然是文字，它也是一个完整命令。进入这个命令，就没有别的路可以走」。**落地**：① **新增 `src/lib/agent/standard-commands.ts`（唯一真相源）** —— **10 条命令**（4 条多步状态机：发布 / 素材线 / AI 制片 / 混合线；3 条单步工具：小红书文案 / 产品海报 / 数字人口播；**3 条 wip**：热点 / 配乐 / 记录待办）＋ `matchStdCommand()`（**去空白后完全相等**）＋ 两段固定话术（"不支持" / "开发中"）——**加/改命令只改这张表，不再往 route.ts 加正则**。② **`route.ts` 加【单一路由器】**（三条出口，先到先赢）：命中 `machine` → **清掉发布线 + 三条成片线的全部草稿（=重来）** 并**强制进状态机（跳过 AI 那一步）**；命中 `tool` → 照旧交给 AI 带工具跑一次（批2 收窄成"只给这一个工具"）；命中 `wip` → 直接回"开发中"；**非命令 + 无进行中流程 → 直接回固定话术，不再让 AI 自由发挥**（这就是"锁死"）。③ **前端**：`FEATURE_TIPS` 删掉「帮我搜一下小红书…」（剩 10 条）＋ **`VF_JSON` 认不出的卡型不再显示协议串原文**（改成固定提示 —— 用户之前看到的 `ai_plan` 假卡原文就是这么来的）。**④ 我主动加的一条例外（可删）**：只读查询 `/视频做得怎么样了\|进度\|做完了吗\|好了吗/` 放行 —— 因为出片入队即作废草稿（`VF_RUN_CLOSE_V1`），否则用户查不了进度。 | `src/lib/agent/standard-commands.ts`（新增）、`src/app/api/agent/chat/route.ts`、`src/app/agent/page.tsx`、`ISSUES.md`（含"新增命令必做两件事"） | ✅ `npx tsc --noEmit`：**新文件 0 错误**、route.ts 仍只剩 2 条历史遗留、page.tsx 无新增（36 条全是历史遗留）；⚠️ **未部署**（用户要求先讨论）；⚠️ **待实测**：发非命令文字应回"标准模式只支持这几条命令…"；发三条成片命令应**各自直接进第 ① 步**（不再蹭旧草稿）；点"热点/配乐/记录待办"应回"开发中" |
| 2026-09-21 | **★修「部署后说『AI 制片帮我做一条视频』被回『请在上面选好 横竖屏/时长/风格』」＋ 混合线抢 AI 入口词** → **用户实测**（上一轮修完入口后立刻暴露的下一层）：入口进得去了（`skipModelStep1` 生效），但**上一轮残留在 `ai_opts` 的草稿把新指令吞了** —— `handleAiLine` 见 `vd` 已存在 → 不进起稿 → 直接落到 `ai_opts` 的兜底话术。**根因：素材线早有"入口词 → 清草稿重来"（`route.ts` 的 `vfIntent`），两条新线都漏了**（又是"只接了一半"）。**改**：① **`VF_AILINE_RESTART_V1`** / **`VF_MIXLINE_RESTART_V1`** —— 两条线各加一条规则：**本线入口词 = 再开一条** → 作废旧草稿、走起稿（只认入口词；「确认 / 下一步 / VF_FORM」**绝不**重置，避免打断进行中的流程）；② **`VF_ENTRY_PRIORITY_V1` 补充**：混合线分派在 AI 制片线**之前**，原来只排除了**素材线**入口词 → 有残留草稿时会把「AI 制片帮我做一条视频」抢走（与"素材线的『确认』被 AI 线抢走"是同一类错、方向相反）→ 补上 AI 入口词也让出。 | `src/lib/agent/vf/vf-aivideo.ts`、`src/lib/agent/vf/vf-mix.ts` | ✅ `npx tsc --noEmit`：两文件 **0 错误**；⚠️ **待用户实测**：说「AI 制片帮我做一条视频」应**立即出主题卡**（不再回"请在上面选好…"）→ 点下一步出分镜卡 → 确认出片 |
