...
- **⑦【成片只有 BGM、没有人声】真因＝百炼音色名被喂给了火山**：表单音色 `longxiaochun` 是**百炼 id**，`chat/route.ts:398` 原样透传 `--speaker` → 百炼链 HTTP 400（项目内同 body 的 `ai-providers.ts:1811` 也不通）→ 火山兜底**不认识这个音色名** → 每镜失败 → 无声片 + dur 停在 AI 值（**64 秒**，而卡片写"预计 182 秒"）。**修**：`★VF_VOICE_MAP_V1` 百炼→火山音色映射 + `★VF_TTSERR_V1` 打印百炼响应体 + 百炼失败即停重试 + `make.py` 失败打首尾行。**验证方法（无需 build）**：`python3 scripts/video-factory/tts.py --text "测试" --speaker longxiaochun --out /tmp/t.mp3` 应输出 `ok=True`
- **待确认**：再跑一条 180 秒 —— 应看到 `[VF] [覆盖率] ≥80%`（门禁放行、「确认出片」出现）+ 成片**有配音、念完整篇、时长接近 180 秒**。
