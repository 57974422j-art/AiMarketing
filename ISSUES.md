...
- 🔴 **微博发布入口错了（2026-09-13 用户实测纠正）**：脚本直接打开 `weibo.com/upload/channel`，但该页面**只有【图片上传框】**（`accept="image/*,.jpg,.jpeg,.bmp,.g"`）——把视频 set_input_files 进去会被静默忽略，**而脚本却报「✅ file input 兜底上传」= 假成功**（日志看成功、微博页面实际空白）。
  **正确入口**：从**微博首页 `weibo.com` 的发送框点「视频」**（发送框有：图片 | 视频 | 话题 | 头条文章）→ 进入真上传流程。
  **教训**：脚本报成功 ≠ 平台真收到——必须回页面核验（本次就是靠"页面还是空白上传页"发现的）。
