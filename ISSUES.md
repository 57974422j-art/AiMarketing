## ★★标准模式锁死（2026-09-21 用户定案 · 已落地"批次 1"）

**规则（用户原话，改之前先读）**
1. 标准模式 = **命令白名单**：只有 `src/lib/agent/standard-commands.ts` 里那 **10 条**
2. **少一个字、错一个字都不执行**（匹配 = 去掉空白后**完全相等**）
3. 每条命令**一个独立状态机**，点哪个进哪个
4. **进去之后不管第几步，只要看到那 10 条里的任意一条 → 立刻重来**（清掉所有流程草稿）
5. 除这些命令外，**发什么文字都回「没有这个功能，请切换去自由模式尝试或联系客服咨询」**
6. 要自由发挥 → 切**自由模式**（`mode:'free'` / `agentMode:'free'`，不受本规则约束）

**当前 10 条命令**

| 类型 | 命令 | 状态 |
|---|---|---|
| `machine`（多步状态机） | 帮我发一个视频 / 用本地成片帮我做一条视频 / **AI 制片帮我做一条视频** / 素材+AI创作做一条视频 | ✅ 可用（命令出现即清掉所有草稿=重来） |
| `tool`（单步） | 帮我写一个小红书文案 / 帮我生成一张产品海报 / 帮我生成一个数字人口播 | ✅ 可用（**批2** 收窄为"只给这一个工具"） |
| `wip`（占位） | 帮我查一下今日热点 / 帮我配一段背景音乐 / 帮我记录一件事：明天要交房租 | ⚠️ 点了回「开发中」 |
| — | ~~帮我搜一下小红书「奶茶」~~ | ❌ 用户定案删除（不做） |

**⚠️ 我主动加的 1 条例外（要严格锁死就删）**：只读查询 `STD_QUERY_RE`（`视频做得怎么样了|进度|做完了吗|好了吗`）放行 —— 因为出片**入队时草稿已作废**（`VF_RUN_CLOSE_V1`），否则用户查不了进度。

**新增一条命令时必须做的两件事**（否则又会"接错线/蹭草稿"）：
1. 加到 `src/lib/agent/standard-commands.ts` 的 `STD_COMMANDS`（**唯一真相源**，别再往 `route.ts` 加正则）
2. 同步前端 `src/app/agent/page.tsx` 的 `FEATURE_TIPS`（**一字不差**）
...
| `VF_AILINE_RESTART_V1` / `VF_MIXLINE_RESTART_V1` | 说「AI 制片帮我做一条视频」→ 回「AI 制片：请在上面选好 横竖屏/时长/风格，点下一步」（**看起来像状态机不进，其实是旧草稿吞了新指令**） | 上一轮残留在 `ai_opts`/`script` 的草稿还在 → `handleAiLine` 见 `vd` 存在就不进起稿 → 落到那一步的兜底话术。**素材线早有"入口词→清草稿重来"，两条新线都漏了** | 两条线各加：**本线入口词 = 再开一条** → 作废旧草稿走起稿（只认入口词，确认/下一步/VF_FORM 绝不重置） |
| `VF_ENTRY_PRIORITY_V1`（补充） | 有残留草稿的**混合线**会把「AI 制片帮我做一条视频」抢走 | 混合线分派在 AI 制片线**之前**，但它只排除了【素材线】入口词，没排除【AI 制片线】入口词 | 混合线补上"AI 制片入口词也让出"（与 AI 线排除素材线词对称） |

| `H3_UA_V1`（第 2 处） | AI 制片出片**必失败**：`[H3] 第 N 镜下载失败: HTTP Error 403: Forbidden` → `生成完成 0/4 镜（通道=中转）` | **产物"下载"这条路没带 UA** —— 上一轮只给 API 调用（`_h3_http`）补了 UA，`_h3_download` 用的是裸 `urlopen` → 同样被 Cloudflare 拦（生成成功、拿不回来） | `_h3_download` 也带 `User-Agent`（`UA_H3`，两处共用一个常量） |
| `VF_AIFAIL_V1` | AI 制片失败后 UI 里冒**一大堆"渲染错误"**，真病因（下载 403）被埋掉；用户原话：「我做的 10 秒视频 好合成干嘛？AI 直接不就出了」 | `make.py` 有"AI 镜全失败 → **自动回退素材合成**"的护栏（本意是别整片挂）；但 **AI 制片没有素材** → 回退后 `bgimage` 没有图片路径 → 渲染崩 → 整片失败 | **AI 制片线（`--source ai` 且无 `--mix`）一镜都没成功 → 不回退、直接失败并说清病因**；混合线保留回退（它本来就有素材画面） |
