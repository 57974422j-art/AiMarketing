...
| 二期 B | create_ai_video 一句话成片 ✅；**/ai-video-tasks 分镜节点链页 ✅**；**提示词库 ✅**：/admin/prompt-sources（6 内置源/定时拉取/封面转存 OSS/补转）+ PromptSource 表 + 标签筛选修复；**发布到素材库 ✅**：PromptTemplate.published（显式发布不乱推）+ admin 发布/下架按钮 + 用户 media-library「💡 提示词」Tab（复制/用这个生成跳 image-generator，本地验证发布3→可见3→下架1→可见2） |
