...
| 二期 B | create_ai_video 一句话成片 ✅；**/ai-video-tasks 分镜节点链页 ✅**（任务列表+节点链：缩略图/预览/单镜重试/prompt编辑/成品播放器/轮询/admin全部视图，storyboard list+PATCH）；**提示词库升级 ✅**（表加 tags/author/coverUrl/imageMode/sourceKey + 多源同步 API + 标签筛选/封面/作者，本地验证 323 条入库）|
