...
async function step6_covers(page, params, log) {
  ...
  try {
    ...
    // 2026-09-10: 只走方向对应的封面入口（竖3:4 / 横4:3）——不再两个都点（两个都开=混乱）
    var _wantLandscape = String(params.coverDir || '').indexOf('landscape') >= 0
    var _idx = _wantLandscape ? 1 : 0   // covers[0]=竖封面3:4 covers[1]=横封面4:3（按脚本收集顺序）
    if (_idx >= covers.length) _idx = 0
    log('  封面方向=' + (_wantLandscape ? 'landscape(横)' : 'portrait(竖)') + ' → 只走入口[' + _idx + ']')
    for (var i = _idx; i <= _idx; i++) {
