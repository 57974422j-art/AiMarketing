...
async def main():
    ...
    MANUAL = '按任务描述执行发布：打开任务里给出的网址，用 upload_file 上传视频文件和封面图，在对应输入框填标题和话题，最后点发布按钮。每步只做一个动作。上传封面时必须先点「设置封面」→「选择封面」（横封面4:3/竖封面3:4）打开封面弹窗，看到弹窗（优质封面示例/上传封面按钮）后再点「上传封面」上传封面文件，最后点「完成」；不要在没打开封面弹窗时直接上传封面。上传后若弹封面裁切/编辑弹窗：先点【保存/保持】再点【完成/确定】，直到弹窗关、封面显示。填标题话题若字数超限，删超出部分再提交，不要反复重试输入。'
