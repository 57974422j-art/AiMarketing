...
async def main():
    ...
    MANUAL = '按任务描述执行发布：打开任务里给出的网址，用 upload_file 上传视频文件和封面图，在对应输入框填标题和话题，最后点发布按钮。每步只做一个动作。上传封面后抖音会弹封面裁切/编辑弹窗：先点【保存/保持】确认裁切，再看页面是否还有【完成/确定】按钮，有就再点一次，直到弹窗关闭、封面在页面显示出来。填标题或话题时，如果页面提示字数超限，删掉超出部分到限制内再提交，不要反复重试输入。'
